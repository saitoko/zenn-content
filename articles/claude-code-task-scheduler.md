...
「これは自動化できるのではないか」と気づいたのは、Claude Codeのスケジュール実行（Scheduled Tasks）という機能を知ったときだった。設定した時刻に、登録したプロンプトを自動実行してくれる仕組みだ。「毎朝6時にZenn記事を公開して」と一度登録しておけば、あとは何もしなくていい。

追加費用は発生しない。スケジュール実行はプランのサブスクリプション料金に含まれている。
...
**プランごとにスケジュールのセッション数に上限がある**のだ。これは公式ドキュメントには未記載で、GitHubのIssueで報告されている情報だ（[Issue #40124](https://github.com/anthropics/claude-code/issues/40124)）。私の環境（Max 5xプラン）では、1日3セッションが上限だった。あなたのプランで何枠利用可能かは確認してみてほしい。

:::message
本記事の情報は執筆時点（2026年4月）のものです。プランや仕様は変更される可能性があります。
:::
...
`adhoc.md` もスケジュール実行とは少し異なる使い方をする。定義したタスクをコミット＆プッシュしたあと、[claude.ai/code/scheduled](https://claude.ai/code/scheduled) から手動で「Run now」をクリックしてトリガーする。スケジュールは不要だが、毎回プロンプトを打ち直したくない繰り返し作業（月次アーカイブなど）の置き場として使っている。
...
`condition` フィールドでよく使うパターンはこの3つだ。

| 値 | 意味 |
|----|------|
| `always` | 毎回実行 |
| `weekday: mon` | 特定の曜日のみ（例: 月曜） |
| `date-range: 2026-04-01..2026-04-30` | 指定期間のみ（連載公開などに便利） |
...
| 機能 | 用途 | 特徴 |
|------|------|------|
| `/schedule` | スケジュール実行の作成・管理 | 「3時にPRレビューして」のように自然言語で登録可能。1枠=1プロンプト。list / update / run も使える |
...
なお、スケジュール実行の最小間隔が1時間である点は[公式ドキュメント](https://code.claude.com/docs/ja/web-scheduled-tasks)の比較表に明記されている。1時間より細かい間隔が必要なリアルタイム監視には `/loop` が適している。
...
これを毎朝の希望時刻にスケジュール登録する。補足: スケジュール時刻はローカルタイムゾーン基準で設定され、実行は予定時刻より数分遅延する場合がある。

[テンプレートリポジトリ](https://github.com/saitoko/claude-code-task-scheduler)を公開している。「Use this template」からリポジトリを作成し、`/schedule` でトリガーを登録するだけで動く。テンプレートにはサンプルタスク・空のhourly.md・adhoc.md・最小構成の例・使い方ガイドが含まれる。

## まとめ

1週間の試行錯誤で見えてきた設計のポイントを共有する。変化は明確だ。

毎朝5回AIに話しかけていた作業コストがゼロになった。Zenn記事の公開も、期限タスクのSlack通知も、週初めの組織ヘルスチェックも、朝起きたらすでに終わっている。
