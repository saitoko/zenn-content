...
**Task Budgets（Claude Opus 4.7 以降）**。Opus 4.7 で追加された `task_budget` パラメータを使うと、エージェントループ全体にトークン予算を割り当てられます。複数のサブエージェントを並列実行すると消費トークンが読みにくくなりますが、このパラメータで上限を明示することでコスト管理が直接的になります。ベータヘッダー `task-budgets-2026-03-13` を付与して `task_budget` を指定すれば、ループが予算内に収まるよう動作します。ただし公式の推奨は「確実な完了を優先する制約付きタスクに限定する」で、品質を最優先にするオープンエンドなエージェント運用には設定しないほうがよいとされています。マルチエージェント構成では「このサブエージェントには予算を設けてコスト上限を引く、このエージェントには設けずに品質優先」という使い分けが重要になりそうです。
